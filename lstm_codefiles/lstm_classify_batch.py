...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 17 12:54:31 2018
...
def pad_and_truncate():
    fp = open('/home/nam/Desktop/AspiringMinds/lstm_datafiles/duplicate_data/lstm_train_duplicate_data.txt','rb')
    X = []
    while True:
            try:
                ...
                break
    wp = open('/home/nam/Desktop/AspiringMinds/lstm_datafiles/duplicate_data/lstm_train_duplicate_scores.txt', 'rb')
    y = []
    while True:
            try:
...
#adam = Adam(lr=3e-4)
train_checkpoint_file = '/home/nam/Desktop/AspiringMinds/lstm_datafiles/duplicate_data/weights/duplicate_class_batch_train_weights.{epoch:02d}--{loss:.2f}.hdf5'
...
early_stop = EarlyStopping(monitor='loss', min_delta=0.005, patience=0, verbose=1, mode='auto')
model.compile(optimizer = rmsprop, loss='categorical_crossentropy', metrics=['accuracy'])
#    model.summary()
#    model.metrics_names

model.fit(X, to_categorical(y, num_classes=7), epochs=NUMBER_EPOCHS, verbose=1, batch_size=BATCH_SIZE, shuffle=True, callbacks=[train_checkpoint, early_stop])
#    print "Set:" + str(old_weights)
    
# serialize model to JSON
model_json = model.to_json()
with open("/home/nam/Desktop/AspiringMinds/lstm_datafiles/duplicate_data/lstm_duplicate_classify_batch_model.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("/home/nam/Desktop/AspiringMinds/lstm_datafiles/duplicate_data/lstm_duplicate_classify_batch_model.h5")
...
#class_weight_dict = dict(enumerate(class_weights))
##rmsprop = RMSprop(lr=LEARNING_RATE, rho=0.9, epsilon=1e-7, decay=0.0)
##sgd = SGD(lr=1e-3, decay=1e-6, momentum=0.9,nesterov=False)
#adam = Adam(lr=1e-3, decay=1e-6)
#train_checkpoint_file = '/home/nam/Desktop/AspiringMinds/lstm_datafiles/lstm_classbatch_weights/class_batch_train_weights.{epoch:02d}--{loss:.2f}.hdf5'
#train_checkpoint = ModelCheckpoint(train_checkpoint_file, monitor='loss', verbose=0, save_best_only=True, save_weights_only=False, mode='min', period=1)
#early_stop = EarlyStopping(monitor='loss', min_delta=0.05, patience=0, verbose=1, mode='auto')
#new_model.compile(optimizer = adam, loss='categorical_crossentropy', metrics=['accuracy'])
#new_model.load_weights("/home/nam/Desktop/AspiringMinds/lstm_datafiles/lstm_batch_model_sgd.h5")
##    model.summary()
##    model.metrics_names
#new_model.fit(X, to_categorical(y, num_classes=7), epochs=NUMBER_EPOCHS, verbose=1, batch_size=BATCH_SIZE,class_weight=class_weight_dict, shuffle=True, callbacks=[train_checkpoint])
#new_model.save_weights("/home/nam/Desktop/AspiringMinds/lstm_datafiles/lstm_batch_model_sgd.h5")
#
