...
# Global variables
VOCAB_SIZE = 25931 #25930, set to avoid collisions when using one_hot
WINDOW_SIZE = 9 
EMBEDDING_SIZE = 200
...
def get_true_sequences():
    ...
    START_END_TOKEN = 0#tokeniser.texts_to_sequences([TOKEN_WORD])
...
#            print y[0]
    ...
    length = len(X)
    vocab_len = 25931 # dont change it to VOCAB_SIZE
    while True:
        batch_true = []
        batch_noisy = []
        batch_labels = []
...
def customized_loss(args):
    ...
    # try dumping this into a file instead
    with open('/home/nam/customized_loss.log', 'a') as logfile:
        logfile.write('ytrue: {}, ypred: {}, loss: {}'.format(y_true, y_pred, loss))
#    print loss
    return loss
...
def CandW_model():
    ...
    shared_embedding = Embedding(output_dim=EMBEDDING_SIZE, input_dim=VOCAB_SIZE, input_length=WINDOW_SIZE, name='embedding_layer',embeddings_initializer='glorot_uniform')
...
TRAIN_SEQ_TOTAL = NEGATIVE_SAMPLES*TRAIN_SEQ_TOTAL
VALID_SEQ_TOTAL = NEGATIVE_SAMPLES*VALID_SEQ_TOTAL
model = CandW_model()

adam = Adam(lr=3e-4)
model.compile(optimizer=adam, loss={'context_layer': lambda y_true, y_pred:y_pred,'output_layer': 'mean_squared_error'}, loss_weights=[ALPHA, 1-ALPHA], metrics={'output_layer': 'accuracy'})
model.summary()
